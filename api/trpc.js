    ...
    function traverse(tree, walker2, origin2) {
      ...
      if (!is_1.isArray(tree)) {
        util_1.forEach(tree, function(subtree, key) {
          return traverse(subtree, walker2, __spreadArray(__spreadArray([], __read(origin2)), __read(pathstringifier_2.parsePath(key))));
        });
        return;
      }
      ...
      if (children) {
        util_1.forEach(children, function(child, key) {
          traverse(child, walker2, __spreadArray(__spreadArray([], __read(origin2)), __read(pathstringifier_2.parsePath(key))));
        });
      }
...
var require_cjs = __commonJS({
  "node_modules/.pnpm/is-what@4.1.16/node_modules/is-what/dist/cjs/index.cjs"(exports) {
    ...
    function isEmptyArray(payload) {
      return isArray2(payload) && payload.length === 0;
    }
    ...
    function isPrimitive(payload) {
      return isBoolean2(payload) || isNull(payload) || isUndefined2(payload) || isNumber2(payload) || isString2(payload) || isSymbol(payload);
    }
...
function jsonlStreamProducer(opts) {
  ...
  if (serialize) stream4 = stream4.pipeThrough(new TransformStream({ transform(chunk, controller) {
    if (chunk === PING_SYM) controller.enqueue(PING_SYM);
    else controller.enqueue(serialize(chunk));
  } }));
  return stream4.pipeThrough(new TransformStream({ transform(chunk, controller) {
...
async function resolveResponse(opts) {
  ...
  try {
    ...
    if (!info.isBatchCall) {
      ...
      switch (info.type) {
        ...
        case "subscription": {
          ...
          const stream4 = sseStreamProducer((0, import_objectSpread23.default)((0, import_objectSpread23.default)({}, config.sse), {}, {
            data: iterable,
    ...
    if (info.accept === "application/jsonl") {
      ...
      const stream4 = jsonlStreamProducer((0, import_objectSpread23.default)((0, import_objectSpread23.default)({}, config.jsonl), {}, {
        maxDepth: Infinity,
...
var tradeRules = mysqlTable("tradeRules", {
  id: int("id").autoincrement().primaryKey(),
...
var adminAuditLogs = mysqlTable("adminAuditLogs", {
  id: int("id").autoincrement().primaryKey(),
...
var createEmailUser = async (email, passwordHash) => {
  return createUser({ email, passwordHash });
};
async function getWallet(userId, asset = "USDT") {
  const db = await getDb();
  if (!db) return null;
...
async function openContractTrade(userId, input) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
  ...
  const [result] = await db.insert(trades).values({
    ...
    settlesAt: new Date(Date.now() + input.durationSeconds * 1e3)
  });
  return result.insertId;
}
var placeTrade = async (data) => {
  return openContractTrade(data.userId, data);
};
async function settleContractTrade(input) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
...
var settleTrade = settleContractTrade;
...
async function adminDashboard() {
  ...
  const [userRows, balanceRows, tradeRows, wonRows, pendingRows] = await Promise.all([
    ...
    db.select({ value: sql`coalesce(sum(balance), 0)` }).from(wallets).where(eq(wallets.asset, "USDT")),
    db.select({ value: sql`count(*)` }).from(trades),
    db.select({ value: sql`count(*)` }).from(trades).where(eq(trades.status, "won")),
    db.select({ value: sql`count(*)` }).from(transactions).where(eq(transactions.status, "pending"))
  ]);
  return {
...
async function setUserFrozen(userId, frozen, adminId) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
  await db.update(users).set({ isFrozen: frozen }).where(eq(users.id, userId));
  return listUsers();
}
async function resetUserPassword(userId, passwordHash, adminId) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
  await db.update(users).set({ passwordHash }).where(eq(users.id, userId));
  return listUsers();
}
async function approveTransaction(transactionId, status, adminId) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
...
async function listTradeRules() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(tradeRules);
}
async function upsertTradeRule(data) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
  await db.insert(tradeRules).values(data).onDuplicateKeyUpdate({ set: data });
  return listTradeRules();
}
async function adjustWallet(userId, asset, amount, type, adminId) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
...
var createSpotOrder = async (data) => {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
  return { success: true };
};
var executeSpotOrder = createSpotOrder;
var transferToUser = async (data) => {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
  return { success: true };
};
...
// server/routers.ts
var assets = z2.enum(["USDT", "BTC", "ETH"]);
var tradeSymbols = z2.enum([
  // Crypto
  "BTCUSDT",
  "ETHUSDT",
  "BNBUSDT",
...
var durations = z2.union([z2.literal(30), z2.literal(60), z2.literal(90)]);
function hashPassword(password, salt = crypto2.randomBytes(16).toString("hex")) {
  const hash = crypto2.pbkdf2Sync(password, salt, 12e4, 64, "sha512").toString("hex");
  return `${salt}:${hash}`;
}
function verifyPassword(password, stored) {
  ...
  return crypto2.timingSafeEqual(Buffer.from(candidate), Buffer.from(original));
}
...
async function getStooqQuote(symbol, displaySymbol) {
...
async function getMarketSnapshot() {
  ...
  const futuresSymbols = [
    ...
    { symbol: "wtiusd", display: "WTIUSD" },
    { symbol: "brentusd", display: "BRENTUSD" }
  ];
  const indicesSymbols = [
    { symbol: "^spxusd", display: "SPX" },
    { symbol: "^ndxusd", display: "NDXUSD" },
    { symbol: "dxy", display: "DXY" }
  ];
  const [cryptoResult, stocksResult, futuresResult, indicesResult] = await Promise.allSettled([
    Promise.allSettled(cryptoSymbols.map((symbol) => getBinancePrice(symbol))),
    Promise.allSettled(stockSymbols.map((s) => getStooqQuote(s.symbol, s.display))),
    Promise.allSettled(futuresSymbols.map((s) => getStooqQuote(s.symbol, s.display))),
    Promise.allSettled(indicesSymbols.map((s) => getStooqQuote(s.symbol, s.display)))
  ]);
  const cryptoRows = cryptoResult.status === "fulfilled" ? cryptoResult.value.flatMap((item) => item.status === "fulfilled" ? [item.value] : []) : [];
  const stockRows = stocksResult.status === "fulfilled" ? stocksResult.value.flatMap((item) => item.status === "fulfilled" ? [item.value] : []) : [];
  const futuresRows = futuresResult.status === "fulfilled" ? futuresResult.value.flatMap((item) => item.status === "fulfilled" ? [item.value] : []) : [];
  const indicesRows = indicesResult.status === "fulfilled" ? indicesResult.value.flatMap((item) => item.status === "fulfilled" ? [item.value] : []) : [];
  return {
    crypto: cryptoRows,
    stocks: stockRows,
    futures: futuresRows,
...
var appRouter = router({
  ...
  auth: router({
    ...
    register: publicProcedure.input(z2.object({ email: z2.string().email(), password: z2.string().min(8) })).mutation(async ({ input }) => {
      ...
      const user = await createEmailUser(input.email.toLowerCase(), hashPassword(input.password));
  ...
  market: router({
    ...
    price: publicProcedure.input(z2.object({ symbol: tradeSymbols.default("BTCUSDT") })).query(({ input }) => getBinancePrice(input.symbol)),
    klines: publicProcedure.input(z2.object({ symbol: tradeSymbols.default("BTCUSDT"), interval: z2.string().default("1m"), limit: z2.number().int().min(10).max(500).default(60) })).query(async ({ input }) => {
      const url2 = `https://api.binance.com/api/v3/klines?symbol=${input.symbol}&interval=${input.interval}&limit=${input.limit}`;
  ...
  wallet: router({
    balances: protectedProcedure.query(({ ctx }) => listWallets(ctx.user.id)),
    transactions: protectedProcedure.query(({ ctx }) => listTransactions(ctx.user.id)),
    createDeposit: protectedProcedure.input(z2.object({ asset: assets, amount: z2.string().regex(/^\d+(\.\d{1,8})?$/) })).mutation(({ ctx, input }) => createTransaction({ userId: ctx.user.id, asset: input.asset, amount: input.amount, type: "deposit", status: "pending", note: "Customer deposit request" })),
    createWithdraw: protectedProcedure.input(z2.object({ asset: assets, amount: z2.string().regex(/^\d+(\.\d{1,8})?$/) })).mutation(({ ctx, input }) => createTransaction({ userId: ctx.user.id, asset: input.asset, amount: input.amount, type: "withdraw", status: "pending", note: "Customer withdraw request" })),
    transfer: protectedProcedure.input(z2.object({ targetUserId: z2.number().int().positive(), asset: assets, amount: z2.string().regex(/^\d+(\.\d{1,8})?$/) })).mutation(({ ctx, input }) => transferToUser({ userId: ctx.user.id, ...input }))
  }),
  spot: router({
    execute: protectedProcedure.input(z2.object({ baseAsset: z2.enum(["BTC", "ETH"]), side: z2.enum(["buy", "sell"]), amount: z2.string().regex(/^\d+(\.\d{1,8})?$/), price: z2.string().regex(/^\d+(\.\d{1,8})?$/) })).mutation(({ ctx, input }) => executeSpotOrder({ userId: ctx.user.id, ...input }))
  }),
  trading: router({
    settings: publicProcedure.query(getSettings),
    rules: publicProcedure.query(listTradeRules),
    history: protectedProcedure.query(({ ctx }) => listTrades(ctx.user.id)),
    placeContract: protectedProcedure.input(z2.object({ symbol: tradeSymbols.default("BTCUSDT"), direction: z2.enum(["UP", "FALL"]), durationSeconds: durations, amount: z2.string().regex(/^\d+(\.\d{1,8})?$/) })).mutation(async ({ ctx, input }) => {
      ...
      return placeTrade({ ...input, userId: ctx.user.id, entryPrice: price.price });
    }),
    settleContract: protectedProcedure.input(z2.object({ tradeId: z2.number().int().positive(), symbol: tradeSymbols.default("BTCUSDT") })).mutation(async ({ ctx, input }) => {
      const price = await getBinancePrice(input.symbol);
      return settleTrade({ tradeId: input.tradeId, closingPrice: price.price, userId: ctx.user.id });
    })
  }),
  admin: router({
    dashboard: adminProcedure.query(adminDashboard),
    users: adminProcedure.query(listUsers),
    transactions: adminProcedure.query(listAllTransactions),
    trades: adminProcedure.query(listAllTrades),
    settings: adminProcedure.query(getSettings),
    updateSettings: adminProcedure.input(z2.object({ profitRate: z2.string().regex(/^0\.\d{1,4}|1\.0000$/), contractsEnabled: z2.boolean(), depositsEnabled: z2.boolean(), withdrawalsEnabled: z2.boolean(), simulationModeEnabled: z2.boolean() })).mutation(({ input }) => updateSettings(input)),
    tradeRules: adminProcedure.query(listTradeRules),
    upsertTradeRule: adminProcedure.input(z2.object({
      durationSeconds: z2.number().int().positive(),
      minAmount: z2.string().regex(/^\d+(\.\d{1,2})?$/),
      profitRate: z2.string().regex(/^0\.\d{1,4}$/),
      label: z2.string().min(1).max(64)
    })).mutation(({ input }) => upsertTradeRule(input)),
    freezeUser: adminProcedure.input(z2.object({ userId: z2.number().int().positive(), frozen: z2.boolean() })).mutation(({ ctx, input }) => setUserFrozen(input.userId, input.frozen, ctx.user.id)),
    resetPassword: adminProcedure.input(z2.object({ userId: z2.number().int().positive(), newPassword: z2.string().min(8) })).mutation(({ ctx, input }) => resetUserPassword(input.userId, hashPassword(input.newPassword), ctx.user.id)),
    adjustWallet: adminProcedure.input(z2.object({ userId: z2.number().int().positive(), asset: assets, amount: z2.string().regex(/^-?\d+(\.\d{1,8})?$/), note: z2.string().min(3) })).mutation(({ ctx, input }) => adjustWallet(input.userId, input.asset, input.amount, ctx.user.id, input.note)),
    reviewTransaction: adminProcedure.input(z2.object({ transactionId: z2.number().int().positive(), status: z2.enum(["approved", "rejected"]) })).mutation(({ ctx, input }) => approveTransaction(input.transactionId, input.status, ctx.user.id)),
    demoTradeOutcome: adminProcedure.input(z2.object({ tradeId: z2.number().int().positive(), closingPrice: z2.string(), outcome: z2.enum(["won", "lost", "draw"]) })).mutation(({ ctx, input }) => settleTrade({ tradeId: input.tradeId, closingPrice: input.closingPrice, forcedStatus: input.outcome, source: "demo_override", adminId: ctx.user.id }))
  })
});
...
async function createContext(opts) {
  ...
  if (!user) {
    user = await authenticateJwt(opts.req.headers.authorization);
  }
  return {
    req: opts.req,
    res: opts.res,
